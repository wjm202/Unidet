...
class Trainer(object):
    def __init__(self, cfg, mode='train'):
        if 'unified_label_file' not in cfg.keys():
            self.metadata=None
        else:
            unified_label_file=json.load(open(cfg.unified_label_file))
            self.metadata = [
        ...
        if cfg.architecture in MOT_ARCH and self.mode in [
            ...
            if self.mode in [
                'eval']:
                self.dataset_list=[]
                for dataset_name in self.cfg.eval_dataset:
                    name=dataset_name[:dataset_name.find('_')]
                    if name=='coco':
                        self.dataset_coco_eval = self.cfg['{}Dataset_{}'.format(capital_mode,name)] = create('{}Dataset_{}'.format(capital_mode,name))()
                        self.dataset_list.append(self.dataset_coco_eval)
                    elif name=='obj365':
                        self.dataset_obj365_eval = self.cfg['{}Dataset_{}'.format(capital_mode,name)] = create('{}Dataset_{}'.format(capital_mode,name))()
                        self.dataset_list.append(self.dataset_obj365_eval)
                    elif name=='oid':
                        self.dataset_oid_eval = self.cfg['{}Dataset_{}'.format(capital_mode,name)] = create('{}Dataset_{}'.format(capital_mode,name))()
                        self.dataset_list.append(self.dataset_oid_eval)
                    elif name=='voc':
                        self.dataset_voc_eval = self.cfg['{}Dataset_{}'.format(capital_mode,name)] = create('{}Dataset_{}'.format(capital_mode,name))()
                        self.dataset_list.append(self.dataset_voc_eval)
                    else:
                ...
                if 'COCOTrainDataset' in self.cfg.keys():
                    self.dataset_coco = self.cfg['COCOTrainDataset'] = create(
                        'COCOTrainDataset')
                else:
                    self.dataset_coco = None
                if 'OBJTrainDataset' in self.cfg.keys():
                    self.dataset_obj365 = self.cfg['OBJTrainDataset'] = create(
                        'OBJTrainDataset')
                else:
                    self.dataset_obj365 = None
                if 'OIDTrainDataset' in self.cfg.keys():
                    self.dataset_oid = self.cfg['OIDTrainDataset'] = create(
                        'OIDTrainDataset')
                else:
                    self.dataset_oid = None
                if 'VOCTrainDataset' in self.cfg.keys():
                    self.dataset_voc = self.cfg['VOCTrainDataset'] = create(
                        'VOCTrainDataset')
                else:
                    self.dataset_voc = None
        ...
        if self.mode == 'train':
            self.loader = create('{}Reader_u'.format(capital_mode))(
                self.dataset_coco,self.dataset_obj365,self.dataset_oid,self.dataset_voc, cfg.worker_num)
    ...
    def _init_metrics(self, validate=False,OUTPUT_DIR=None,dataset_name=None):
        ...
        if self.cfg.metric == 'COCO' or self.cfg.metric == "SNIPERCOCO":
            ...
            if dataset_name is not None and OUTPUT_DIR is not None:
                output_eval = os.path.join(OUTPUT_DIR + "inference_{}".format(dataset_name))
            ...
            # annotation file
            clsid2catid = {v: k for k, v in self.dataset_list[self.cfg.eval_dataset.index(dataset_name)].catid2clsid.items()}
            # when do validation in train, annotation file should be get from
            # EvalReader instead of self.dataset(which is TrainReader)
            if self.mode == 'train' and validate:
                eval_dataset = self.cfg['EvalDataset_{}'.format(dataset_name[:dataset_name.find('_')])]
                eval_dataset.check_or_download_dataset()
                anno_file = eval_dataset.get_anno()
                dataset = eval_dataset
            else:
                dataset = self.dataset_list[self.cfg.eval_dataset.index(dataset_name)]
                anno_file = self.dataset_list[self.cfg.eval_dataset.index(dataset_name)].get_anno()
    ...
    def train(self, validate=False):
        ...
        for epoch_id in range(self.start_epoch, self.cfg.epoch):
            ...
            for step_id, data in enumerate(self.loader):
                # print('data_time:{}'.format((time.time() - iter_tic)))
                ...
                # print('model_time:{}'.format((time.time() - model_time_before)))
            ...
            if validate and is_snapshot:
                ...
                for dataset_name in self.cfg.eval_dataset:
                    name=dataset_name[:dataset_name.find('_')]
                    if name=='coco':
                        self.dataset_coco_eval = self.cfg['{}Dataset_{}'.format('Eval',name)] = create('{}Dataset_{}'.format('Eval',name))()
                        self.dataset_list.append(self.dataset_coco_eval)
                    elif name=='obj365':
                        self.dataset_obj365_eval = self.cfg['{}Dataset_{}'.format('Eval',name)] = create('{}Dataset_{}'.format('Eval',name))()
                        self.dataset_list.append(self.dataset_obj365_eval)
                    elif name=='oid':
                        self.dataset_oid_eval = self.cfg['{}Dataset_{}'.format('Eval',name)] = create('{}Dataset_{}'.format('Eval',name))()
                        self.dataset_list.append(self.dataset_oid_eval)
                    elif name=='voc':
                        self.dataset_voc_eval = self.cfg['{}Dataset_{}'.format('Eval',name)] = create('{}Dataset_{}'.format('Eval',name))()
                        self.dataset_list.append(self.dataset_voc_eval)
                    else:
                        raise ValueError("not support."
            )
                for i in range(len(self.dataset_list)):
                    self.model.set_eval_dataset(self.cfg.eval_dataset[i])
                    self.dataset_eval=self.dataset_list[i]
                    self._eval_batch_sampler = paddle.io.BatchSampler(
                        self.dataset_eval, batch_size=self.cfg.EvalReader['batch_size'])
                    reader_name = '{}Reader'.format('Eval')
                    if self.cfg.metric == 'VOC':
                        self.cfg[reader_name]['collate_batch'] = False
                    self.eval_loader = create(reader_name)(self.dataset_eval, self.cfg.worker_num,
                                                        self._eval_batch_sampler)
                    if validate and Init_mark == False:
                        Init_mark = True
                        self._init_metrics(validate=validate,OUTPUT_DIR=self.cfg.OUTPUT_DIR,dataset_name=self.cfg.eval_dataset[i])
                        self._reset_metrics()
                    Init_mark = False
                    with paddle.no_grad():
                        self.status['save_best_model'] = True
                        self._eval_with_loader(self.eval_loader,self.cfg.eval_dataset[i])
    ...
    def _eval_with_loader(self, loader,eval_dataset=None):
        sample_num = 0
        tic = time.time()
        self._compose_callback.on_epoch_begin(self.status)
        ...
        if 'OUTPUT_DIR' in self.cfg.keys():
            output_folder = os.path.join(
                self.cfg.OUTPUT_DIR, "inference_{}".format(eval_dataset))
        else:
            output_folder=None
        for metric in self._metrics:
            metric.accumulate()
    ...
    def evaluate(self):
        for i in range(len(self.dataset_list)):
            self.model.set_eval_dataset(self.cfg.eval_dataset[i])
            self.dataset=self.dataset_list[i]
            self._eval_batch_sampler = paddle.io.BatchSampler(
                self.dataset, batch_size=self.cfg.EvalReader['batch_size'])
            reader_name = '{}Reader'.format(self.mode.capitalize())
            if self.cfg.metric == 'VOC':
                self.cfg[reader_name]['collate_batch'] = False
            self.loader = create(reader_name)(self.dataset, self.cfg.worker_num,
                                                self._eval_batch_sampler)
            self._init_metrics(OUTPUT_DIR=self.cfg.OUTPUT_DIR,dataset_name=self.cfg.eval_dataset[i])
            self._reset_metrics()
            if self.cfg.get('fleet', False):
            ...
            with paddle.no_grad():
                self._eval_with_loader(self.loader,self.cfg.eval_dataset[i])
    ...
    def predict(self,
        ...
        anno_file = self.dataset.get_anno()
        if self.model.eval_dataset==-1 and self.metadata is not None:
            clsid2catid={}
            catid2name={}
            for i in range(len(self.metadata)):
                clsid2catid[i]=i
                catid2name[i]=self.metadata[i]
        else:  
            clsid2catid, catid2name = get_categories(
                self.cfg.metric, anno_file=anno_file)
            ...
            for outs in results:
                ...
                for i, im_id in enumerate(outs['im_id']):
                    ...
                    bbox_res = batch_res['bbox'][start:end] \
                            if 'bbox' in batch_res else None
                    image = visualize_results(
                        image, bbox_res, int(im_id), catid2name, draw_threshold)
                    self.status['result_image'] = np.array(image.copy())
                    if self._compose_callback:
                        self._compose_callback.on_step_end(self.status)
                    # save image with detection
