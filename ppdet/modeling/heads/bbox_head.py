...
class BBoxHead(nn.Layer):
    ...
    def __init__(self,
        ...
        self.use_sigmoid_ce = True
        self.use_eql_loss = False
        self.use_fed_loss = False
        self.fed_loss_num_cat = 50
        self.pos_parents = True
        self.hierarchy_ignore = True
    ...
    def _load_class_hierarchy(self):
        ...
        if self.hierarchy_ignore:
            if not os.path.exists('datasets/oid/annotations/challenge-2019-label500-hierarchy-list.json'):
                return
            # print('Loading', cfg.MODEL.ROI_BOX_HEAD.HIERARCHY_PATH)
            hierarchy_data = json.load(
                open('datasets/oid/annotations/challenge-2019-label500-hierarchy-list.json', 'r'))
    ...
    def get_loss(self,
        ...
        """
        num_class=kwargs.get('num_class', self.num_class)
        ...
        if self.use_sigmoid_ce:
            loss_cls = self.sigmoid_cross_entropy_loss(
                scores, tgt_labels, use_advanced_loss=use_advanced_loss)
            loss_bbox[cls_name] = loss_cls
    ...
    def sigmoid_cross_entropy_loss(
        ...
        target = paddle.zeros([B, C + 1])
        target=paddle.nn.functional.one_hot(gt_classes, num_classes=target.shape[-1])
        target = target[:, :C]
        weight = 1
        ...
        if use_advanced_loss and (self.hierarchy_weight is not None) and \
            ...
            if self.pos_parents:
                target = paddle.mm(target, self.is_parents) + target # B x C
        ...
        return paddle.sum(cls_loss * weight) / B
