...
import math
__all__ = ['CascadeTwoFCHead', 'CascadeXConvNormHead', 'CascadeHead','SplitHead']
...
class CascadeHead(BBoxHead):
    ...
    def __init__(self,
                 ...
                 hierarchy_ignore=False,
                 hierarchy_pos_parents=False):
        ...
        label_map = json.load(
            open('dataset/label_spaces/learned_mAP.json', 'r'))['label_map']
        # add background class
        self.dataset_names = ['coco', 'objects365', 'oid']
        self.dataset_inds = {i: paddle.to_tensor(
            [x for x in label_map[d]] + [num_classes]) \
            for i, d in enumerate(self.dataset_names)}

        self.back_map = {}
        for i, d in enumerate(self.dataset_names):
            self.back_map[i] = paddle.zeros([num_classes + 1],dtype='int32')
    ...
    #unidet add 
        ...
        self.pos_parents = hierarchy_pos_parents
        self.hierarchy_ignore = hierarchy_ignore
        
        self.freq_weight = None
        hierarchy_weight = self._load_class_hierarchy()
    ...
    def _load_class_hierarchy(self):
        hierarchy_weight = None
        if self.hierarchy_ignore:
            if not os.path.exists('dataset/oid/annotations/challenge-2019-label500-hierarchy-list.json'):
                return
            # print('Loading', cfg.MODEL.ROI_BOX_HEAD.HIERARCHY_PATH)
            hierarchy_data = json.load(
                open('dataset/oid/annotations/challenge-2019-label500-hierarchy-list.json', 'r'))
            ...
            catid2continous = {x['id']: continousid.index(x['id']) \
                for x in categories}
            C = len(categories)
            is_parents = paddle.zeros((C + 1, C),dtype="float32")
            is_chirlds = paddle.zeros((C + 1, C), dtype="float32")
            for c in categories:
    ...
    def forward(self, body_feats=None, rois=None, rois_num=None, inputs=None,eval_dataset=-1):
        """
        ...
        if self.training:
            ...
            dataset_sources = list(map(int,inputs['dataset_source']))
            targets_list = [targets]
            self.assigned_rois = (rois, rois_num)
            self.assigned_targets = targets
        else:
            dataset_sources = [eval_dataset for _ in range(inputs['image'].shape[0])]
        ...
        for i in range(self.num_cascade_stages):
            if i > 0:
                # pred_bbox=[paddle.to_tensor(np.load('rois.npy'))]
            ...
            # rois=[paddle.to_tensor(np.load('rois.npy'))]
            rois_feat = self.roi_extractor(body_feats, rois, rois_num)
            if self.training:
                rois_feat.register_hook(lambda grad: grad / self.num_cascade_stages)
            bbox_feat = self.head(rois_feat, i)
            scores = self.bbox_score_list[i](bbox_feat)
            deltas = self.bbox_delta_list[i](bbox_feat)
            self.unified_map_back=True
            if (self.unified_map_back or not self.training) and dataset_source != -1:            
                if self.training:
                    scores = paddle.index_select(x=scores, index=self.dataset_inds[dataset_source],axis=-1)
                    for j in range(len(inputs['gt_class'])):
                        # targets_list[-1][0][j] = paddle.to_tensor(np.load('proposal_gt_class.npy'))
                        fg_inds = targets_list[-1][0][j] != self.num_classes
                        targets_list[-1][0][j][fg_inds] = \
                            self.back_map[dataset_source][targets_list[-1][0][j][fg_inds]].astype('int64') \
                                if targets_list[-1][0][j][fg_inds].shape[0]!=0 else targets_list[-1][0][j][fg_inds]
                        bg_inds =  targets_list[-1][0][j] == self.num_classes
                        targets_list[-1][0][j][bg_inds] = scores.shape[1] - 1
                else:
                        scores = paddle.index_select(x=scores, index=self.dataset_inds[dataset_source],axis=-1)
    ...
    def get_prediction(self, head_out_list):
        ...
        """
        pred_list = []
        scores_list = [F.sigmoid(head[0]) for head in head_out_list]
...
class SplitHead(BBoxHead):
    ...
    def __init__(self,
        ...
        for i in range(num_cascade_stages):
            score_name = 'bbox_score_stage{}'.format(i)
            delta_name = 'bbox_delta_stage{}'.format(i)
            # num_classes_list=[80,365,500]
            bbox_score = self.add_sublayer(
                score_name,
                nn.LayerList(nn.Linear(
                    in_channel,
                    num_classes + 1,
                    weight_attr=paddle.ParamAttr(initializer=Normal(
                        mean=0.0, std=0.01)),
                    bias_attr = paddle.ParamAttr(initializer=Constant(bias_value))) for num_classes in self.num_classes_list),
                    )
            bbox_delta = self.add_sublayer(
                delta_name,
                nn.Linear(
    ...
    def forward(self, body_feats=None, rois=None, rois_num=None, inputs=None,eval_dataset=-1):
        ...
        inputs (dict{Tensor}): The ground-truth of image
        """
        # self.num_classes= self.num_classes_list[inputs['dataset_source'][0]]
        body_feats=body_feats[:3]
        targets = []
        if self.training:
            rois, rois_num, targets = self.bbox_assigner(
                ...
                inputs,
                add_gt_as_proposals=self.add_gt_as_proposals[0],
                num_classes=self.num_classes_list[inputs['dataset_source'][0]])
            dataset_sources = list(map(int,inputs['dataset_source']))
            targets_list = [targets]
            self.assigned_rois = (rois, rois_num)
            self.assigned_targets = targets
        ...
        for i in range(self.num_cascade_stages):
            ...
            bbox_feat = self.head(rois_feat, i)
            if dataset_source!=-1 :
                scores = self.bbox_score_list[i][dataset_source](bbox_feat)
            else:
                raise ValueError('dataset_source should one of [0, 1, 2]')
            deltas = self.bbox_delta_list[i](bbox_feat)
        ...
        if self.training:
            ...
            for stage, value in enumerate(zip(head_out_list, targets_list)):
                ...
                loss_stage = self.get_loss(
                    ...
                    loss_normalize_pos=self.loss_normalize_pos,
                    use_advanced_loss=(dataset_source==self.openimage_index),
                    num_class=self.num_classes_list[inputs['dataset_source'][0]])
                for k, v in loss_stage.items():
                    loss[k + "_stage{}".format(
                        stage)] = v * self.stage_loss_weights[stage]
            return loss, bbox_feat
